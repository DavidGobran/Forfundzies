...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let launchProject = (name, active) => { 
        
         return new Promise((resolve, reject) => {
            pool.query("UPDATE Projects SET active=true WHERE name=?", [name], (error, result) => {
                console.log(result);
                if (error) { return reject(error); }
    ...
    try {
        ...
        if (exists) {
            console.log("E4")
            // INSERT
            const inserted = await launchProject(info.name);
            console.log("E6")
            if (inserted) {
                response.statusCode = 200;
            } else {
                 response.statusCode = 400;
                 response.error = "Couldn't update " + info.name;
            }
           
        } else {
            response.statusCode = 400;
            response.error = "Project does not exists";
