...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let insertDesigner = (email, password, role, name) => {
        return new Promise((resolve, reject) => {
            pool.query("INSERT INTO Credentials (email,password,role,name) VALUES(?,?,?,?)", [email, password, role, name], (error, rows) => {
                if (error) { return reject(error); }
    ...
    try {
        ...
        console.log("e2")
        if (!exists) {
            const inserted = await insertDesigner(info.email, info.password, info.role, info.name);
            if (inserted) {
