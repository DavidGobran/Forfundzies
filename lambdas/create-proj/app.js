...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let InsertProject = (name, designer, story, genre, goal, deadline) => {
        return new Promise((resolve, reject) => {
            pool.query("INSERT INTO Projects (name,designer,story,genre,goal,deadline) VALUES(?,?,?,?,?,?)", [name, designer, story, genre, goal, deadline], (error, rows) => {
                if (error) { return reject(error); }
                if ((rows) && (rows.length == 1)) {
    ...
    try {
        ...
        if (exists) {
            ...
            console.log("E4")
            // INSERT
            const inserted = await InsertProject(info.name, info.designer, info.story, info.genre, info.goal, info.deadline);
